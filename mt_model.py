...
class Encoder(nn.Module):
    def __init__(self, vocab_size, embed_size, pretrained_embed, enc_hidden_size, dec_hidden_size, dropout=0.2):
        super(Encoder, self).__init__()
        self.embed = nn.Embedding(vocab_size, embed_size)
        self.embed.weight.data.copy_(torch.from_numpy(pretrained_embed))
    ...
    def forward(self, x, lengths):
        embedded = self.dropout(self.embed(x))  # batchsize x len setence x embedding dim
        ...
        # 【batch, seq, 2 * enc_hidden_size】 要输出的embedding就是这里的out
...
class Decoder(nn.Module):
    ...
    def create_mask(self, x_len, y_len):
        ...
        # 将超出自身序列长度的元素设为False
        x_mask = (torch.arange(max_x_len.item())[None, :].to(x_len.device) < x_len[:,
                                                                             None]).float()  # [batch,max_x_len]
        y_mask = (torch.arange(max_y_len.item())[None, :].to(x_len.device) < y_len[:,
                                                                             None]).float()  # [batch,max_y_len]
